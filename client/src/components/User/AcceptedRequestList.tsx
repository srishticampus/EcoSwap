...
function AcceptedRequestList({ url }) {
  ...
  const fetchPendingRequests = async () => {
    try {
      const res = await axios.get(`http://localhost:8000/swap/accepted/${userId}`);
      if (res.data.success) {
...
//   };
  ...
  return (
    <div className="mt-24 p-6 rounded-lg shadow-md max-w-4xl mx-auto my-10">
      <h2 className="text-2xl font-bold mb-6">Swap Requests History</h2>
